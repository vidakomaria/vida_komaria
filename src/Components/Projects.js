...
export default function Projects(){
    const fastwork = imageProjects.find(item => item.id === 2);
    const simita = imageProjects.find(item => item.id === 3);
    const buku = imageProjects.find(item => item.id === 4);
    return(
        <div className="cont-projects">
            ...
            <div className="project-list">
                {dataProjects.map((project, index) => {
                    let images = [];
                    if (project.id === 2 && fastwork) {
                        images = fastwork.url;
                    }
                    if (project.id === 3 && simita) {
                        images = simita.url;
                    }
                    if (project.id === 4 && buku) {
                        images = buku.url;
                    }
                    ...
                    return (
                        <div className="card" style={{ flexDirection:'column' }} key={index}>
                            ...
                            {project.id === 2 && (
                                <div>
                                    <Link to="/project/fastwork">
                                        <button className="btnFastwork">See Detail</button>
                                    </Link>
                                </div>
                            )}
                        </div>
